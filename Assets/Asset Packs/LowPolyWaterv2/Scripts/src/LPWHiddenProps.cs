...
namespace LPWAsset
{
    public static class LPWHiddenProps
    {
        ...
        {
            // don't scale prefabs 
            if (PrefabUtility.GetCorrespondingObjectFromSource(_target) == null && PrefabUtility.GetCorrespondingObjectFromSource(_target) != null)
                return;

            if (_target.material == null || !_target.material.HasProperty("_TransformScale_"))
                return;
